...
const [sendStatus, setSendStatus] = useState(null);// variable définissant le status de l'envoi 
  ...
  return (
    <main className="carteEmail">
      {/* zone du titre */}
      <div className="titreCarteEmail">
        <div>Message au prestataire</div>
        {/* icone d'annulation */}
        <CancelTwoToneIcon
          className="cancel"
          sx={iconeStyle}
        ...
        />
          {/* icone d'envoi d'un email : l'icone devient verte si success sinon rouge */}
        <SendTwoToneIcon
          className={`send ${sendStatus === "success" ? "success" : ""} ${
            sendStatus === "error" ? "error" : ""
          }`}
          sx={iconeStyle}
          onDoubleClick={sendMessage}
        />
      </div>
