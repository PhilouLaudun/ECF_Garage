...
export const loginFitness = createAsyncThunk(
  "utilisateur/login",
  async (data) => {
...
export const listeUtilisateur = createAsyncThunk(
  "utilisateur",
  async () => {
...
const utilisateurSlice = createSlice({
  ...
  extraReducers:(builder) => {
    builder.addCase(loginFitness.fulfilled, (state, { payload }) => {
      ...
      if (okay) {
        ...
        if (payload.user.Qualite === "1") {// permet de lever le flag pour les options réservées à l'administrateur
          state.flagRole = true;
        }
